...
> ```
> Fix: add `stage('Build') { steps { sh 'mvn clean verify -DskipTests' } }` before the Sonar stage.
...
pipeline {
    agent any
    tools {
        // Maven must be configured under Manage Jenkins → Tools → Maven installations
        maven 'maven'
    }
    environment {
    ...
    stages {
        ...
        stage('Build') {
            // compile first — SonarQube Java analyzer requires .class files in target/classes
            steps { sh 'mvn clean verify -DskipTests' }
        }
        stage('SonarQube Analysis') {
            steps {
                withSonarQubeEnv('SonarQube') {

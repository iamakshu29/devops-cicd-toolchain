...
```
Jenkins + SonarQube Pipeline — What Was Missed & How We Fixed It
1. Maven was not available in Jenkins
Error: mvn: not found
Problem:
Jenkins didn't have a Maven installation available to the pipeline agent.

Fix:
Configured Maven under:
Manage Jenkins → Tools → Maven installations
The configured installation name was:
maven
...
2. sonar:sonar plugin wasn't being resolved

Error:
No plugin found for prefix 'sonar'

Problem:
Maven couldn't resolve the sonar plugin using the short prefix.

Fix:
Instead of:
mvn sonar:sonar

we used the fully qualified plugin:
mvn org.sonarsource.scanner.maven:sonar-maven-plugin:5.7.0.6970:sonar

This also gave us an explicit plugin version.



3. SonarQube couldn't analyze Java because the project wasn't compiled (BIGGEST PROBLEM WHICH I THINK)
...
Problem:
We were running Sonar analysis before compiling the Java project.

The pipeline was effectively:
Checkout
   ↓
Sonar Analysis ❌

Sonar's Java analyzer needed the compiled .class files.

Fix:
Added a build stage before Sonar:
stage('Build') {
    steps {
...
4. Also u didnt tell me the flag for proper logs if I am getting errors
```
...
Checking status of SonarQube task '32777cf6-f81e-475c-8f7c-9e0826b00c95' on server 'SonarQube'
SonarQube task '32777cf6-f81e-475c-8f7c-9e0826b00c95' status is 'IN_PROGRESS'
SonarQube task '32777cf6-f81e-475c-8f7c-9e0826b00c95' status is 'SUCCESS'
SonarQube task '32777cf6-f81e-475c-8f7c-9e0826b00c95' completed. Quality gate is 'OK'
[Pipeline] }
[Pipeline] // timeout
```

```
Also do devOps need to define the quality gates?
Like what devOps actually do ...just add the steps in Jenkins or anything else
```